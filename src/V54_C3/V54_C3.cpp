...
// Constant Variables
const int SW_Version = 54;
const int HW_Version = 20;
...
void loadConfiguration(const char *filename, Config &config)
{
    ...
    if (SPIFFS.begin(true))
    {
        ...
        StaticJsonDocument<2000> doc;   // using JsonDocument throws error
...
void task_process_buffer(void *pvParameters)
{
    ...
    {                                                                           // let first process
            ...
            // For commands other than 5, the aux byte is the slot number.
            if (command == 5 && (slot_type_atom.load() != dataDecoded[5]))
            {
                // Command 5 = Set Slot Type AND Slot Number
...
void loop()
{
    ...
    // Main Loop Function - get/set data based on slot_type
    switch (slot_type)
    {
    case 1:
        val = pdo2.load();
    ...
    // Handle LED state if needed
    unsigned long currentMillis = millis();
    if (ble_enabled)
    {
        //Blink Blue for BLE active
